 ...
 *
 * Generates presigned GET download URL for private Cloudflare R2 object retrieval.
 * Endpoint: POST /functions/v1/r2-download-url
 */

import { serve } from 'https://deno.land/std@0.168.0/http/server.ts';
import { createClient } from 'https://esm.sh/@supabase/supabase-js@2.39.0';
import { corsHeaders, generateR2PresignedDownloadUrl } from '../_shared/r2Client.ts';
...
serve(async (req: Request) => {
  ...
  // 3. Tenant Boundary Check: Enforce storageKey starts with workspaces/{workspaceId}/
  ...
  // 4. Verify workspace ownership via RLS
  ...
  try {
    const presigned = await generateR2PresignedDownloadUrl(storageKey, 3600);
    ...
    console.error('[R2 Presigned Download URL Error]:', err);
    return new Response(
      JSON.stringify({ error: err instanceof Error ? err.message : 'Gagal menghasilkan R2 download URL.' }),
      {
        status: 500,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      }
