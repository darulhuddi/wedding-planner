...
import { openGoogleDrivePicker, getGoogleConfig } from '../../services/storage/googleDrivePicker';
import { X, Upload, Link as LinkIcon, Image as ImageIcon, Loader2, HardDrive, Sparkles, CheckCircle2 } from 'lucide-react';
...
export interface AddEditInspirationModalProps {
  ...
  onSaveGoogleDrive?: (
    ...
    details?: { title?: string; note?: string; tags?: string; sourceUrl?: string }
  ) => Promise<void>;
...
export const AddEditInspirationModal: React.FC<AddEditInspirationModalProps> = ({
  ...
  const [imageMode, setImageMode] = useState<'google_drive' | 'upload' | 'url'>('google_drive');
  const [selectedDriveFiles, setSelectedDriveFiles] = useState<GoogleDriveSelectedFile[]>([]);
  ...
  const handleOpenDrivePicker = async () => {
    ...
    try {
      ...
      if (selected && selected.length > 0) {
        setSelectedDriveFiles(selected);
        if (selected.length === 1 && !title) {
          setTitle(selected[0].fileName.replace(/\.[^/.]+$/, ''));
        }
      }
    } catch (err: unknown) {
      console.error('[AddEditInspirationModal] Google Picker error:', err);
      const msg = err instanceof Error ? err.message : 'Gagal membuka Google Drive Picker.';
      setFormError(msg);
    } finally {
      setIsPickerLoading(false);
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!categoryOnlyMode) {
      if (imageMode === 'google_drive' && selectedDriveFiles.length === 0 && !initialData) {
        setFormError('Silakan pilih foto dari Google Drive kamu terlebih dahulu.');
        return;
      }
    ...
    try {
      if (imageMode === 'google_drive' && selectedDriveFiles.length > 0 && onSaveGoogleDrive && !initialData) {
        await onSaveGoogleDrive(selectedDriveFiles, category, {
          ...
          {!categoryOnlyMode && !initialData && (
            <>
              ...
              <div className="space-y-2">
                ...
                {/* Mode Tabs */}
                <div className="grid grid-cols-3 gap-1.5 p-1 bg-ivory-100 rounded-xl border border-beige-300 text-xs font-medium">
                  <button
                    type="button"
                    onClick={() => setImageMode('google_drive')}
                    className={`flex items-center justify-center gap-1.5 py-2 rounded-lg transition-all cursor-pointer ${
                      imageMode === 'google_drive'
                ...
                {imageMode === 'google_drive' && (
                  <div className="bg-emerald-50/60 border border-emerald-200/80 rounded-2xl p-4 text-center space-y-3">
                    <div className="inline-flex items-center gap-1 bg-white border border-emerald-200 text-emerald-800 text-[10px] font-bold px-2.5 py-0.5 rounded-full shadow-2xs">
                    ...
                    <div>
                      <h4 className="font-serif text-sm font-bold text-charcoal">
                        Hubungkan Google Drive kamu
                      </h4>
                      <p className="text-[11px] text-charcoal-500 max-w-xs mx-auto mt-0.5">
                        Foto tetap tersimpan aman di Google Drive milik kamu dan tidak memenuhi penyimpanan WedSiap.
                      </p>
                    </div>

                    {selectedDriveFiles.length > 0 ? (
                      <div className="bg-white border border-emerald-300 rounded-xl p-3 flex items-center justify-between text-xs text-left">
                        <div className="flex items-center gap-2 min-w-0 pr-2">
                          <CheckCircle2 className="w-4 h-4 text-emerald-600 shrink-0" />
                          <div className="min-w-0">
                            <span className="font-semibold text-charcoal block truncate">
                              {selectedDriveFiles.length} Foto Terpilih
                            </span>
                            <span className="text-[10px] text-charcoal-400 block truncate">
                              {selectedDriveFiles.map((f) => f.fileName).join(', ')}
                            </span>
                          </div>
                        </div>
                        <button
                      ...
                      <button
                        ...
                        className="inline-flex items-center justify-center gap-2 px-4 py-2.5 bg-emerald-700 hover:bg-emerald-800 text-white font-semibold text-xs sm:text-sm rounded-xl shadow-2xs transition-all cursor-pointer min-h-touch disabled:opacity-50"
                      >
                        {isPickerLoading ? (
                    ...
                    {!googleConfig.isConfigured && (
                      <p className="text-[10px] text-amber-700 bg-amber-50 border border-amber-200 p-2 rounded-lg">
                        API Google Drive belum aktif di .env. Gunakan Device Upload atau Paste URL sebagai alternatif.
                      </p>
                    )}
                  </div>
                )}

                {/* Option 2: Upload dari Perangkat (WebP optimized) */}
                {imageMode === 'upload' && (
                      ...
                      <label className="flex flex-col items-center justify-center gap-2 cursor-pointer py-3">
                        ...
                        <div>
                          ...
                          <p className="text-[11px] text-charcoal-400 mt-0.5">
                            Otomatis dikompresi ke WebP (maks 1600×1600) sebelum disimpan
                          </p>
                        </div>
                        <input
                          type="file"
